  ...
  if (user) {
    ...
    await user.save(function (err, User) {
      if (err) {
        ...
        const token = jwt.sign(userJson, config.secret, {
          expiresIn: config.tokenLife
        })
        const refreshToken = jwt.sign(userJson, config.refreshTokenSecret, {
...
exports.userLogin = async function (req, res) {
  ...
  if (!req.body.email || !req.body.password) {
    return res.status(400).json({
      "message": "You must send the email and the password for login",
      "status": 0
    });
  }
